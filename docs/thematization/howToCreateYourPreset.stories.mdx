...
Создайте директорию для пресетов, внутри сделайте папки для каждого модификатора — такие же, как в [компоненте Theme]. Например, так:
...
Настройте переменные в CSS (их иногда называют _дизайн-токенами_). За образец можно взять CSS-файлы из [компонента Theme].

> Лучше давать переменным семантические имена, так их проще контролировать и применять

### Цвета `_color`

Это набор переменных, которые применяются к разным частям компонентов. Значения переменных наследуются от базовых `base` цветовых переменных.
...
import { Theme } from 'nx-design/Theme';
