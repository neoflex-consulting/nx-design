...
**Файл загружается**. Вместо иконки с расширением файла покажите [индикатор загрузки](#индикаторы-загрузки), если хотите — процент, на который файл загрузился. При наведении появляется кнопка для отмены загрузки с иконкой-крестиком (`Close`).
...
**Файл успешно загрузился**. В этом состоянии уже точно известно расширение файла — можно показать соответствующую иконку (если она есть). Дополнительно можно вывести метаинформацию о файле: размер, дату, автора или что-то ещё. При наведении появляется кнопка для удаления файла с иконкой корзины (`Delete`).
...
import { Attachment } from 'nx-design/Attachment';
