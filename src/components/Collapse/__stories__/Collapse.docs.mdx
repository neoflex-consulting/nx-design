...
} from './examples/CollapseExampleIconCustomIcon/CollapseExampleIconCustomIcon'; import {
  CollapseExampleIconLeftSideBadge, CollapseExampleIconLeftSideButton
} from "./examples/CollapseExampleIconLeftSideBadge/CollapseExampleIconLeftSideBadge";
...
За дополнительный элемент отвечает свойства `rightSide` и `leftSide`.

`rightSide` можно использовать, только если иконка у коллапса расположена слева `iconPosition={"left"}`.

`leftSide` можно использовать, только если иконка у коллапса расположена справа `iconPosition={"right"}`.

<CollapseExampleIconRightSideBadge />
<CollapseExampleIconRightSideButton />

<CollapseExampleIconLeftSideBadge />
<CollapseExampleIconLeftSideButton />

## Иконка

По умолчанию у коллапса иконка-стрелочка: `ChevronDown` — в закрытом виде, `IconArrowUp` — в открытом.
...
| [`leftSide?`](#дополнительный-элемент)      | `React.ReactNode`                                                                                  | -                     | Дополнительный контент, который можно поместить рядом с заголовком коллапса |
