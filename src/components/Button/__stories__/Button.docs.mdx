...
import { ButtonExampleReactNode } from './examples/ButtonExampleReactNode/ButtonExampleReactNode';
import { ButtonExampleOtherIcons } from './examples/ButtonExampleOtherIcons/ButtonExampleOtherIcons'
...
Всего в дизайн-системе четыре типа кнопок: `primary`, `secondary`, `ghost`, `clear` и `link`. Этого обычно хватает, чтобы расставить
...
<Button label="Комментировать" view="secondary" iconLeft={Edit} />
<Button label="Редактировать" view="secondary" iconRight={Chat} onlyIcon />
```
...
<Button label="Настройки" view="ghost" iconLeft={Settings} />
<Button label="Фильтр" view="ghost" iconLeft={Filter} onlyIcon />
```
...
<Button label="Скопировать" view="clear" iconLeft={Copy} />
<Button label="Добавить в Избранное" view="clear" iconLeft={Mark} onlyIcon />
```
...
Это полноценная кнопка с прозрачным фоном и подчеркиванием при наведении. Используйте её, когда необходимо добавить ссылку на другой компонент или страницу. Также эти кнопки удобно использовать в Paginator, при переключении между страницами.

```ts
<Button label="Ссылка" view="link" />
<Button label="Назад" view="link" iconLeft={ArrowBackward} />
<Button label="Сохранить" view="link" iconLeft={Save} onlyIcon />
```
...
<Button label="Продолжить" disabled iconRight={ArrowRight} />
<Button label="Добавить" disabled iconRight={Plus} onlyIcon />
```
...
Для отображения расчетного значения на кнопке, используйте ReactNode в label.
Например: Отображение перевода через i18n. Добавление Link и прочее...

```ts
<Button label={t('create')} {{ width: '260px' }} />
<Button
    view={'link'}
...
<ButtonExampleReactNode />

### Использование сторонних библиотек с иконками

Например: https://ant.design/components/icon/ или https://iconpark.oceanengine.com/
...
<ButtonExampleOtherIcons />

## Список свойств

| Свойство                         | Тип или варианты значения                                                                 | По умолчанию              | Описание                                                                         |
| -------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------- |
| [`label`](#текст-на-кнопке)      | `string`, `number`, `React.ReactNode`                                                     | -                         | Текст на кнопке                                                                  |
| [`view`](#внешний-вид)           | `'clear', 'primary', 'secondary', 'ghost', 'link'                                         | `primary`                 | Внешний вид                                                                      |
...
| [`iconLeft`](#кнопка-с-иконкой)  | `React.ReactNode` , `React.FC<IconProps>`                                                 | -                         | Иконка слева                                                                     |
| [`iconRight`](#кнопка-с-иконкой) | `React.ReactNode` , `React.FC<IconProps>`                                                 | -                         | Иконка справа                                                                    |
