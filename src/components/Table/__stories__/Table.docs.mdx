...
```tsx
<Table columns={columns} rows={rows} emptyRowsPlaceholder={<Typography>Здесь пока нет данных</Typography>} />
```
...
const App = () => {
  return <Table columns={columns} rows={rows} emptyRowsPlaceholder={<Typography>Пусто</Typography>} />;
};
```
...
import { Table, SortByProps } from 'nx-design/Table';
