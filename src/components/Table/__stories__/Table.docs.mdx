...
- [Внешний вид](#внешний-вид)
  ...
  - [Разделение строк с границами между столбцами](#разделение-строк-с-границами-между-столбцами)
...
[Посмотреть пример](?path=/story/components-table--with-sticky-header)
...
[Посмотреть пример](?path=/story/components-table--with-sticky-column)
...
import React from 'react';
import { Table } from 'nx-design/Table';
import { Checkbox } from 'nx-design/Checkbox';
...
```tsx
import React from 'react';
import { Table, TableColumn } from 'nx-design/Table';
import { Text } from 'nx-design/Text';
...
К таблице вы можете добавить верхнее меню. За это отвечает свойство `withHeaderMenu`.

Для наполнения верхнего меню, необходимо указать свойства в Table:

- `rightSide` — наполнение меню справа
- `leftSide` — наполнение меню слева
- `stopIsProgressLineVisible` — состояние ProgressLine в таблице
- `nameResetAllFilters` — Всплывающая подсказка на кнопке закрыть все фильтры (если вы используете фильтры)


Структура `rightSide` и `leftSide`:

- `name` — Готовый пресет с компонентами для левого и правого меню. На текущий момент есть два: `RightSide`, `LeftSide`.

- `RightSide` — Содержит кнопку скрыть/показать колонки с соответствующим меню.
- `props` — Свойства, которые передаются в пресет
- `nameApplyHiddenColumn` — label на кнопке скрыть/показать колонки в `RightSide`
- `nameButtonColumnSettings` — Всплывающая подсказка на кнопке скрыть/показать колонки в `RightSide`
- `overrideHiddenColumns` — функция принимает колонки, которые сейчас должны быть отображены в таблице в `RightSide`


- `LeftSide` — Содержит кнопки добавить строку и обновить таблицу.
- `props` — Свойства, которые передаются в пресет
- `nameButtonAddColumn` — Всплывающая подсказка на кнопке добавить строку в `LeftSide`
- `nameButtonRefresh` — Всплывающая подсказка на кнопке обновить таблицу в `LeftSide`
- `onClickButtonRefresh` — Обработчик при клике на кнопку обновить таблицу в `LeftSide`
- `buttonAddType` — Тип кнопки добавления. Поле типа ButtonAddPropType. Возможные значения: 'button', 'menu'

- `buttonAddItems: ContextMenuItemDefault[]` - элементы выпадающего меню при клике на кнопку добавить строку в `LeftSide` для `buttonAddType == menu`
- `onClickButtonAdd` — Обработчик при клике на кнопку добавить строку в `LeftSide` для `buttonAddType == button`
- `buttonAddColumnVisible` — Видимость кнопки добавления в `LeftSide`
...
При нажатии на кнопку обновить таблицу, под Верхним меню появляется ProgressLine. Выключить его можно, передав в таблицу состояние через `stopIsProgressLineVisible` или нажав на кнопку обновить таблицу повторно.
В примере ProgressLine включается и выключается на timeout = 4 секунды.
...
[Посмотреть пример](?path=/story/components-table--with-header-menu)
...
import { Table, SortByProps, TableColumn } from 'nx-design/Table';
...
```tsx
import { TableTextFilter, TableNumberFilter, TableChoiceGroupFilter } from 'nx-design/Table';
...
| `TableTextFilter`        | `{ items: Array<{ name: string, value: any }>; withSearch: boolean; nameSearch: string; nameSelectAll: string; nameReset: string; nameApply: string; nameCancel: string; nameNotFound: string }` | Фильтрация по нескольким значениям |
...
### Разделение строк с границами между столбцами

`separateRows` добавит границы между строками расширенные.
...
import { Text } from 'nx-design/Text/Text';
import { Badge } from 'nx-design/Badge';
import { cnMixSpace } from 'nx-design/MixSpace';
import { Table, TableColumn } from 'nx-design/Table';
...
| [`separateRows?`](#разделение-строк-с-границами-между-столбцами)  | `boolean`                                                                                                                    | `false`                                           | Отображение границ между строками расширенные                                                                         |
| [`nameResetAllFilters?`](#плейсхолдер-кнопка-сбросить-все-фильтры)| `string`                                                                                                                     | `Сбросить все`                                    | Плейсхолдер на кнопке Сброить все фильтры                                                                             |
