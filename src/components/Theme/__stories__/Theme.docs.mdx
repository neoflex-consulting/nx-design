...
- [Модификаторы](#модификаторы)
  - [Цвета](#цвета)
    - [Цветовые схемы в NX Design](#цветовые-схемы-в-nx-design)
  ...
  - [Тени](#тени)
- [Пресеты](#пресеты)
  - [Пресеты в NX Design](#пресеты-в-nx-design)
...
Файлы хранится в директории компонента `Theme`, в папках с именами модификаторов. В дизайн-системе NX Design это выглядит так:
...
Пресет — это `preset`, обязательное свойсто компонента `Theme`. В NX Design три пресета:

- `presetGpnDefault` — включает светлую цветовую схему,
- `presetGpnDark` — тёмную,
- `presetGpnDisplay` — акцентную.

[Подробнее о цветовых схемах](#цветовые-схемы-в-nx-design)
...
#### Цветовые схемы в NX Design

В дизайн-системе NX Design 3 цветовые схемы:
...
`gpnDark`, `gpnDefault`, `gpnDisplay` и `gpnDatagram` — это значения модификатора `_color`, с их помощью можно обращаться к нужной цветовой схеме, подключать, [переключать](#зачем-нужны-пресеты)
и получать переменные из CSS.

#### Как использовать цветовые схемы

Обычно проекты, сделанные на основе NX Design, используют темы Default и Dark с возможностью переключения между ними.
...
```tsx
import React from 'react';
import { Theme, presetGpnDefault, presetGpnDark, presetGpnDisplay } from 'nx-design/Theme';
...
Если вы работаете с NX Design и делаете проект для Неофлекс, постарайтесь
...
В дизайн-системе NX Design три пресета:
...
[Подробнее о цветовых схемах в NX Design](#цветовые-схемы-nx-design)

Большая часть компонентов дизайн-системы использует основную цветовую схему. Но не все.

#### Компоненты в инвертной схеме

Некоторые компоненты нужно вынести на другой информационный слой, но так, чтобы не перекрывать остальной интерфейс. Например, [Tooltip](http://cloud2:6006//?path=/story/components-tooltip--tooltip-positioned-by-anchor-story).
...
компоненты, на которые пользователю стоит обратить внимание: [Notification](/?path=/story/components-Notification--playground),
[Alert](/?path=/story/components-alert--playground)
...
В самом начале работы подключите компонент `Theme` и укажите пресет в корневом компоненте — так, чтобы все компоненты nx-design были вложены в компонент `Theme`.
