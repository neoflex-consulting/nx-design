...
class Day extends Component<Props, State> {
  ...
  render() {
    ...
    const _moment = this.state.moment;
    const firstDay:number = _moment.clone().date(1).day();
    const endOfThisMonth:number = _moment.clone().endOf('month').date();
    const endOfLastMonth:number = _moment.clone().subtract(1, 'month').endOf('month').date();
