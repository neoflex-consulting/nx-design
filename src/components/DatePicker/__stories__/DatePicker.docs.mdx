...
import { DatePickerExampleShowPicker } from './examples/DatePickerExampleShowPicker/DatePickerExampleShowPicker';
...
import { DatePickerExampleMulti } from './examples/DatePickerExampleMulti/DatePickerExampleMulti'; import {
  DatePickerExampleDirection
} from "./examples/DatePickerExampleDirection/DatePickerExampleDirection";
...
`DatePicker` сделан из двух других компонентов — [TextField](?path=/docs/components-textfield--playground) (поле ввода) и [Calendar](?path=/docs/components-calendar--playground) (календарь),
...
- [Формат календаря](#формат-календаря)
...
- [Свойства поля ввода](#свойства-поля-ввода)
  ...
  - [Раположение](#расположение-поля)
- [Обработка ошибок](#обработка-ошибок)
- [Определение положения элемента (z-index)](#определение-положения-элемента-z-index)
- [Импорт](#импорт)
...
```tsx
const DatePickerExampleTypeCalendar = () => {
  const [value, setValue] = useState<Date | null>(null);
...
За формат даты отвечают два свойства:

- `formatMask` — маска для отображения даты,
- `separator` — разделитель, который нужно использовать.

`formatMask` по умолчанию зависит от `type`:

- `type=date` или `type=date-range` — `formatMask='yyyy-MM-dd'`
- `type=date-time` — `formatMask='yyyy-MM-dd HH:mm:ss'`
...
Чтобы задать формат, который используется в США, укажите `formatMask='MM/dd/yyyy HH:mm:ss'`.
...
За формат календаря отвечает свойство showPicker:

- `'date'` - Выбор дня, месяца и года `formatMask='yyyy-MM-dd'`
- `'month'` - Выбор месяца и года `formatMask='yyyy-MM'`
- `'year'` - Выбор только года `formatMask='yyyy'`
...
За статус отвечает свойство `status`. Варианты: `success`, `error` и `warning`.
...
Чтобы изменить расположение календаря относительно поля ввода, используйте свойство `direction`

Значение по-умолчанию: `downStartLeft`
...
## Определение положения элемента (z-index)

Для изменения положения элемента Календарь в DataPicker необходимо использовать стиль `zIndex`.

Значение `zIndex` по-умолчанию `zIndex={typeof style?.zIndex === 'number' ? style.zIndex + 1 : undefined}`
...
| [`showPicker?`](#формат-календаря)                                         | `'date'`, `'month'`, `'year'`                                                                                                                                       | `date`                | Тип календаря: выбор полной даты (День + Месяц + Год), выбор Месяц + Год или выбор Год                                 |
| [`direction?`](#расположение-поля)                                         | `'downStartLeft'`, `'upStartLeft'`  , `'downStartRight'` , `'upStartRight'` , `'leftStartUp'` , `'leftStartDown'` , `'rightStartUp'` , `'rightStartDown'`           | `downStartLeft`       | Расположение календаря относительно поля ввода                                                                         |
...
| `minDate?`                                                                 | `Date`                                                                                                                                                              | `01.01.0001 00:00:00` | Ограничение выбора по меньшей дате, см. [Calendar](/?path=/docs/components-calendar--playground)                       |
| `maxDate?`                                                                 | `Date`                                                                                                                                                              | `31.12.9999 23:59:59` | Ограничение выбора по большей дате, см. [Calendar](/?path=/docs/components-calendar--playground)                       |
| `events?`                                                                  | `Date[]`                                                                                                                                                            | -                     | События на календаре, см. [Calendar](/?path=/docs/components-calendar--playground)                                     |
| `calendarView?`                                                            | `'classic'`, `'book'`, `'slider'`                                                                                                                                   | `'classic'`           | Вид календаря: одна страничка, две странички или слайдер, см. [Calendar](/?path=/docs/components-calendar--playground) |
| `locale?`                                                                  | `Locale`                                                                                                                                                            | `ruLocale`            | Локализация, см. [Calendar](/?path=/docs/components-calendar--playground)                                              |
...
| `status?`                                                                  | `''error', 'success', 'warning'`                                                                                                                                    | -                     | Статус поля, см. [TextField](?path=/docs/components-textfield--playground)                                             |
...
| [`renderAdditionalControls?`](?path=/docs/components-calendar--playground) | `CalendarAdditionalControlRenderProp`                                                                                                                               | -                     | Дополнительные конторолы для выбора даты                                                                               |
