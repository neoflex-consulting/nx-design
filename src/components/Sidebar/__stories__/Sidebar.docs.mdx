...
За активацию пространства подложки отвечает свойство `includeSpace`. По умолчанию
пространство заблокировано для нажатия.

<SidebarExampleNoOverlay />

## Контейнер

По-умолчанию родительским контейнером является `window.document.body`

Для изменения контейнера необходимо использовать `document`
