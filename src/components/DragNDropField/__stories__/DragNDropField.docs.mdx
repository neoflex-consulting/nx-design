...
<DragNDropField>
  {({ openFileDialog }) => (
    <>
      <Typography>Пример с Render Props, открывает окно для выбора файла из дочернего блока</Typography>
      <br />
...
<DragNDropField accept="image/*" onDropFiles={(files) => console.log(files)}>
  <Typography>Сюда можно перетаскивать только картинки, зато любые (ну почти)</Typography>
...
<DragNDropField maxSize={1 * 1024 * 1024}>
  <Typography>Сюда можно перетаскивать файлы размером 1 Мб или меньше</Typography>
</DragNDropField>
```
