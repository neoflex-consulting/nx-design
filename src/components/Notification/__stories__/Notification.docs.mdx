...
import { NotificationExampleHiddenMessage } from './examples/NotificationExampleHiddenMessage/NotificationExampleHiddenMessage';
...
- [Иконка](#иконка)
- [Кнопки](#кнопки)
- [Работа со списком уведомлений](#список-уведомлений)
- [Скрытие строк в сообщении](скрытие-строк)
- [Таймер](#таймер)
...
## Скрытие строк в сообщении

Используйте встроенную функцию `hiddenMessage` для управления списком уведомлений.

Оно сокращает сообщения до двух строк. В конце сообщения ставит три точки и появляется кнопка Подробнее.

Чтобы изменить надпись на кнопку Подробнее используйте свойства: `titleButtonMoreOn` и `titleButtonMoreOff`

<NotificationExampleHiddenMessage/>
...
Свойство `autoClose` принимает числовое значение. Если оно будет равно нулю, то автоматическое выключение не сработает.

При вклдючении работы таймера вы можете использовать иконку, которую сами выберите или иконку с таймером.
Для этого использйте свойство `withAutoCloseTimer`.
...
const generateNotification = (withAutoCloseTimer: boolean) => () => {
...
const handleAlertAdd = generateNotification(true);
const handleNormalAdd = generateNotification(false);
...
| hiddenMessage?     | `boolean`                                                              | `false`      | Сокращает сообщение до двух строк                                |
| titleButtonMoreOn? | `string`                                                               | `Подробнее`  | Текст кнопки от hiddenMessage                                    |
| titleButtonMoreOff?| `string`                                                               | `Скрыть`     | Текст кнопки от hiddenMessage                                    |
