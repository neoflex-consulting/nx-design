...
В основе модуля — компонент [Button](?path=/docs/components-button--playground) в модификациях: `view_clear`, `size_m`, `form_round` с [иконкой](?path=/story/components-icons--playground) размера `m`.
